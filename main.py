...
TIMEOUT_SECONDS = 3.0
TOLERANCE = 100  # There is a buffer zone of anything shorter than 100mm, the sensor will not trip (this is to add a tolerance to the sensor
...
def init_sensors(vl53):
    """This initialization code was taken from https://docs.circuitpython.org/projects/vl53l0x/en/latest/examples.html#multiple-vl53l0x-on-same-i2c-bus
...
def init_baseline(vl53, baseline_distance):
    """Set the baseline distance to the floor for each sensor.
        ...
        We take 5(abritrary number, this could probably be lower) readings and average them to get a better baseline reading.
    ...
    while True:
        try:
            ...
            for i in range(len(distances)):
                ...
                # Check #1: If the distance is less than the buffer zone, something is detected
                if distance < baseline_distance[i] - TOLERANCE:
                    ...
                    if not sensor_states[i]["is_active"]:
                        ...
                        # Add this event to the event history, but remove old events if the list is full
                        event_history.append({"sensor": i, "action": "set", "time": timestamp})
            ...
            if (sensor_states[0]["set_time"] is not None and
                ...
                set_events = []
                for event in event_history: #isolate just the set events
                    if event["action"] == "set":
                        set_events.append(event)

                if len(set_events) >= 2:
                    #sort the set events based on timestamp -- we had help from Github copilot here for sorting a list of dictionaries by a value in the dictionary, there seems to be other ways
                    #but this is easiest for readbility
